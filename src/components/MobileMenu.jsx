...
const MobileMenu = ({ isOpen, onClose, handleLinkClick }) => {
    ...
    return createPortal(
        <div className="lg:hidden" role="dialog" aria-modal="true">
            {/* Background backdrop */}
            <div
                className="fixed inset-0 z-[9998] bg-gray-900/80"
                onClick={onClose}
                aria-hidden="true"
            />

            {/* Mobile Menu Panel */}
            <div className="fixed inset-y-0 left-0 z-[9999] w-full overflow-y-auto bg-white px-6 py-6 sm:max-w-sm sm:ring-1 sm:ring-gray-900/10">
                {/* Header */}
                        ...
                        <div className="space-y-2 py-6">
                            {navItems.map((item) => (
                                <Link
                                    key={item.path}
                                    to={item.path}
                                    className={`-mx-3 block rounded-lg px-3 py-2 text-base font-semibold leading-7 ${isActive(item.path)
